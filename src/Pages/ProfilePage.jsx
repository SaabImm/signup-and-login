...
import { useParams } from "react-router-dom"; // Add this if not already
...
export default function ProfilePage({ user }) {
  ...
  const { id } = useParams(); // Get ID from URL if viewing other profiles
  ...
  // Filter personal details based on view permissions
    ...
    <>
      <div className="min-h-screen bg-gray-900 pb-16 relative">
        ...
        <div className="w-3/4 mx-auto -mt-20 bg-gray-800/80 backdrop-blur-xl 
          ...
          {/* Avatar - always visible if exists */}
          ...
          {/* User main info - filtered by permissions */}
          <div className="flex-1 space-y-2">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              {/* Role Badge - visible if role field is visible */}
              {isVisible('role') && (
                <span
              ...
              {/* Verification Badge - always visible? or make permission-based? */}
              <span
        ...
        <div className="w-3/4 mx-auto mt-12 bg-gray-800/80 backdrop-blur-xl 
          ...
          <div className="grid grid-cols-2 gap-8 text-gray-200">
            ...
            {isVisible('dateOfBirth') && (
              <DetailBox 
                label="Date of Birth" 
                value={displayUser?.dateOfBirth ? new Date(displayUser.dateOfBirth).toLocaleDateString() : ''} 
              />
            )}
          </div>
        </div>
